...
const overwriteBigNumberFunction_1 = __importDefault(require("./overwriteBigNumberFunction"));
const utils_1 = require("ethers/utils");
const utils_2 = require("../utils");
const solidity = (chai, utils) => {
    const { Assertion } = chai;
    Assertion.overwriteMethod('equal', (_super) => overwriteBigNumberFunction_1.default('eq', 'equal', _super, utils));
    Assertion.overwriteMethod('eq', (_super) => overwriteBigNumberFunction_1.default('eq', 'equal', _super, utils));
    Assertion.overwriteMethod('above', (_super) => overwriteBigNumberFunction_1.default('gt', 'above', _super, utils));
    Assertion.overwriteMethod('below', (_super) => overwriteBigNumberFunction_1.default('lt', 'below', _super, utils));
    Assertion.overwriteMethod('least', (_super) => overwriteBigNumberFunction_1.default('gte', 'at least', _super, utils));
    Assertion.overwriteMethod('most', (_super) => overwriteBigNumberFunction_1.default('lte', 'at most', _super, utils));
    ...
    Assertion.addMethod('changeBalance', function (wallet, balanceChange) {
        ...
        const derivedPromise = utils_2.getBalanceChange(subject, wallet)
            .then((actualChange) => {
            this.assert(actualChange.eq(utils_1.bigNumberify(balanceChange)), `Expected "${wallet.address}" to change balance by ${balanceChange} wei, ` +
    ...
    Assertion.addMethod('changeBalances', function (wallets, balanceChanges) {
        ...
        const derivedPromise = utils_2.getBalanceChanges(subject, wallets)
            .then((actualChanges) => {
            const walletsAddresses = wallets.map((wallet) => wallet.address);
            this.assert(actualChanges.every((change, ind) => change.eq(utils_1.bigNumberify(balanceChanges[ind]))), `Expected ${walletsAddresses} to change balance by ${balanceChanges} wei, ` +
