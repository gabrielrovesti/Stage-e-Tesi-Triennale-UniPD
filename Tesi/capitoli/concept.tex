...
\section{Obiettivi e requisiti}\label{sec:analisi-obiettivi}
...
\begin{itemize}
    ...
    \item Desiderabili:
        \begin{itemize}
            \item \underline{\textit{D01}}: implementare una \glsfirstoccur{\gls{dappg}} (o \textit{DApp}) tramite le librerie \glsfirstoccur{\gls{ethersjsg}} oppure \glsfirstoccur{\gls{web3jsg}}, utilizzando uno \textit{smart contract} di esempio;
...
Nel codice che viene utilizzato come libreria,
...
In questa sezione, all'utente viene presentata una lista di film e può scegliere di prenotare uno di questi. Ciascun film possiede una sua valutazione;
...
di massima (\textit{username}, \textit{email}, data di nascita) e successivo \textit{login} (utilizzando il solo \glsfirstoccur{\gls{didg}}), verificando che 
...
In particolare, possiamo dettagliare in qualità di attori:
\begin{itemize}
\item \textbf{utente non autenticato}: attore che indica un utente che non ha ancora effettuato l'accesso o la registrazione all'interno del sistema;
\item \textbf{utente autenticato}: attore che indica un utente che ha effettuato l'accesso all'interno del sistema e ha la possibilità di effettuare operazioni di prenotazione, verificando la propria età in base al tipo di film.
\end{itemize}
...
\usecasepost{L'utente ha completato la registrazione e può effettuare il \textit{login}}
...
\begin{usecase}{1.1}{Inserimento \textit{username}}\label{uc:registrazione-username}
  \usecaseactors{Utente non autenticato}
  \usecasepre{L'utente ha avviato l'applicazione web e ha aperto la pagina di registrazione}
  \usecasedesc{L'utente deve inserire uno \textit{username} per registrarsi presso l'applicazione web}
  \usecasepost{L'utente ha inserito lo \textit{username} e può procedere con la registrazione}
  \usecasemain{}
  
  \begin{enumerate}
    \item L'utente inserisce il proprio \textit{username}.
  \end{enumerate}
  
\end{usecase}

\begin{usecase}{1.2}{Inserimento \textit{email}}\label{uc:registrazione-email}
  \usecaseactors{Utente non autenticato}
  \usecasepre{L'utente ha avviato l'applicazione web e ha aperto la pagina di registrazione}
  \usecasedesc{L'utente deve inserire una \textit{email} per registrarsi presso l'applicazione web}
...
\begin{usecase}{1.3}{Inserimento \textit{DID}}\label{uc:registrazione-did}
  \usecaseactors{Utente non autenticato}
  \usecasepre{L'utente ha avviato l'applicazione web e ha aperto la pagina di registrazione}
  \usecasedesc{L'utente deve inserire il proprio \textit{DID} per registrarsi presso l'applicazione web}
  \usecasepost{L'utente ha inserito il proprio \textit{DID} e può procedere con la registrazione}
  \usecasemain{}
...
\begin{usecase}{2}{Visualizzazione errore \textit{email} già esistente}\label{uc:registrazione-email-esistente}
  \usecaseactors{Utente non autenticato}
  \usecasepre{L'utente ha inserito una \textit{email} già esistente}
  \usecasedesc{L'utente deve inserire una \textit{email} diversa per registrarsi presso l'applicazione web}
  \usecasepost{L'utente ha inserito una \textit{email} diversa e può procedere con la registrazione}
  \usecasemain{}
  
  \begin{enumerate}
    \item L'utente visualizza un messaggio di errore che lo informa che la \textit{email} inserita è già presente nel sistema.
  \end{enumerate}
\end{usecase}

\begin{usecase}{3}{Visualizzazione errore \textit{DID} già esistente}\label{uc:registrazione-did-esistente}
  \usecaseactors{Utente non autenticato}
  \usecasepre{L'utente ha inserito un \textit{DID} già esistente}
  \usecasedesc{L'utente deve inserire un \textit{DID} diverso per registrarsi presso l'applicazione web}
  \usecasepost{L'utente ha inserito un \textit{DID} diverso e può procedere con la registrazione}
  \usecasemain{}
...
\begin{usecase}{4.1}{Inserimento \textit{DID}}\label{uc:autenticazione-did}
  \usecaseactors{Utente non autenticato}
  \usecasepre{L'utente ha avviato l'applicazione web, non è già autenticato e ha aperto la pagina di autenticazione}
  \usecasedesc{L'utente deve inserire il proprio \textit{DID} associato alle proprie credenziali per autenticarsi presso l'applicazione web}
  \usecasepost{L'utente ha inserito il proprio \textit{DID} e può procedere con l'autenticazione}
  \usecasemain{}
...
\begin{usecase}{5}{Visualizzazione errore \textit{DID} errato}\label{uc:autenticazione-did-errato}
  \usecaseactors{Utente non autenticato}
  \usecasepre{L'utente ha inserito un \textit{DID} errato}
  \usecasedesc{L'utente deve inserire un \textit{DID} corretto per autenticarsi presso l'applicazione web}
  \usecasepost{L'utente ha inserito un \textit{DID} non esistente e può procedere con l'autenticazione}
  \usecasemain{}
  
  \begin{enumerate}
    \item L'utente visualizza un messaggio di errore che lo informa che il \textit{DID} inserito non è corretto.
  \end{enumerate}
\end{usecase}
...
\begin{usecase}{12.1}{Inserimento nuovo \textit{username}}\label{uc:modifica-username}
  \usecaseactors{Utente autenticato}
  \usecasepre{L'utente è autenticato e intende modificare il proprio \textit{username}}
  \usecasedesc{L'utente vuole modificare lo \textit{username} associato al proprio \textit{account}}
  \usecasepost{L'utente ha inserito il proprio \textit{username} correttamente}
  \usecasemain{}
...
\begin{usecase}{12.2}{Inserimento nuova \textit{email}}\label{uc:modifica-email}
  \usecaseactors{Utente autenticato}
  \usecasepre{L'utente è autenticato e intende modificare la propria \textit{email}}
  \usecasedesc{L'utente vuole modificare la mail associata al proprio \textit{account}}
  \usecasepost{L'utente ha modificato la propria \textit{email} correttamente}
  \usecasemain{}
...
\begin{usecase}{13}{Eliminazione \textit{account}}\label{uc:eliminazione-account}
  \usecaseactors{Utente autenticato}
  \usecasepre{L'utente è autenticato e intende eliminare il proprio \textit{account}}
  \usecasedesc{L'utente vuole eliminare il proprio \textit{account} dal sistema}
  \usecasepost{L'utente ha eliminato il proprio \textit{account} e il sistema ha eliminato tutti i dati ad esso associati}
  \usecasemain{}
...
\begin{usecase}{14}{Logout}\label{uc:logout}
  ...
  \usecasedesc{L'utente vuole effettuare il \textit{logout} dal sistema}
  \usecasepost{L'utente ha effettuato il \textit{logout} e il sistema ha terminato la sessione dell'utente, non permettendo più l'accesso alle funzionalità riservate}
  \usecasemain{}
...
\begin{longtable}{|C{2.0cm}|C{12.0cm}|C{2.0cm}|}
\hline
